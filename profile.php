	...
	if ( !$functions->get_config('guests_can_view_profiles') && $session->sess_info['user_id'] == 0 ) {
		...
		if ( $db->num_rows($result) || $own_profile ) {
			...
			$template->parse('profile', array(
				...
				'msnm'          => $lang['MSNM'],
				'msnm_v'        => htmlentities(stripslashes($profiledata['msnm'])),
				'yahoom'        => $lang['YahooM'],
				'yahoom_v'      => htmlentities(stripslashes($profiledata['yahoom'])),
				'aim'           => $lang['AIM'],
				'aim_v'         => htmlentities(stripslashes($profiledata['aim'])),
				'icq'           => $lang['ICQ'],
				'icq_v'         => htmlentities(stripslashes($profiledata['icq'])),
