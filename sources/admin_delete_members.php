...
if ( !empty($_GET['id']) && valid_int($_GET['id']) ) {
	...
	if ( $memberdata['id'] && $memberdata['id'] != $session->sess_info['user_id'] ) {
		...
		if ( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
			...
			$content .= '<p><label><input type="checkbox" name="banemail" value="1" />  '.$lang['DeleteMembersBanEmail'].' </label><input type="text" name="email" size="35" maxlength="255" value="'.$memberdata['email'].'" /></p>';
