...
class functions {
	...
	// Resync statistics
	//
	function resync($what, $id=NULL) {
		
		global $db;
		
		if ( $what == 'global_forums' ) {
			
			//
			// Get forum count
			//
			if ( !($result = $db->query("SELECT id FROM ".TABLE_PREFIX."forums")) )
				$this->usebb_die('SQL', 'Unable to get forum count!', __FILE__, __LINE__);
			$forum_count = $db->num_rows($result);
			
			//
			// Set forum count
			//
			if ( !($result = $db->query("UPDATE ".TABLE_PREFIX."stats SET content = ".$forum_count." WHERE name = 'forums'")) )
				$this->usebb_die('SQL', 'Unable to set forum count!', __FILE__, __LINE__);
			
		} elseif ( $what == 'global_topics' ) {
			
			//
			// Get topic count
			//
			if ( !($result = $db->query("SELECT id FROM ".TABLE_PREFIX."topics")) )
				$this->usebb_die('SQL', 'Unable to get topic count!', __FILE__, __LINE__);
			$topic_count = $db->num_rows($result);
			
			//
			// Set topic count
			//
			if ( !($result = $db->query("UPDATE ".TABLE_PREFIX."stats SET content = ".$topic_count." WHERE name = 'topics'")) )
				$this->usebb_die('SQL', 'Unable to set topic count!', __FILE__, __LINE__);
			
		} elseif ( $what == 'global_posts' ) {
			
			//
			// Get post count
			//
			if ( !($result = $db->query("SELECT id FROM ".TABLE_PREFIX."posts")) )
				$this->usebb_die('SQL', 'Unable to get post count!', __FILE__, __LINE__);
			$post_count = $db->num_rows($result);
			
			//
			// Set post count
			//
			if ( !($result = $db->query("UPDATE ".TABLE_PREFIX."stats SET content = ".$post_count." WHERE name = 'posts'")) )
				$this->usebb_die('SQL', 'Unable to set post count!', __FILE__, __LINE__);
			
		} elseif ( $what == 'global_members' ) {
			
			//
			// Get member count
			//
			if ( !($result = $db->query("SELECT id FROM ".TABLE_PREFIX."users")) )
				$this->usebb_die('SQL', 'Unable to get member count!', __FILE__, __LINE__);
			$member_count = $db->num_rows($result);
			
			//
			// Set member count
			//
			if ( !($result = $db->query("UPDATE ".TABLE_PREFIX."stats SET content = ".$member_count." WHERE name = 'members'")) )
				$this->usebb_die('SQL', 'Unable to set member count!', __FILE__, __LINE__);
