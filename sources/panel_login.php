...
if ( preg_match(USER_PREG, $_POST['user']) && preg_match(PWD_PREG, $_POST['passwd']) && strlen($_POST['passwd']) >= 5 ) {
	...
	if ( !$session->sess_info['user_id'] ) {
		...
		$_SERVER['HTTP_REFERER'] = ( !empty($_SERVER['HTTP_REFERER']) && !preg_match("/act=(activate|login)/", $_SERVER['HTTP_REFERER']) ) ? $_SERVER['HTTP_REFERER'] : 'index.php';
		$_SESSION['refere_to'] = ( !empty($_SESSION['referer']) && !preg_match("/act=(activate|login)/", $_SESSION['referer']) ) ? $_SESSION['referer'] : $_SERVER['HTTP_REFERER'];
