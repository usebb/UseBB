...
if ( preg_match(EMAIL_PREG, $_POST['email']) && ( empty($_POST['avatar']) || preg_match(IMG_PREG, $_POST['avatar']) ) && ( empty($_POST['website']) || preg_match(WEB_PREG, $_POST['website']) ) ) {
	...
	$template->parse('edit_profile', 'panel', array(
		...
		'occupation_input' => '<input type="text" size="50" maxlength="255" name="occupation" value="'.htmlentities(stripslashes($session->sess_info['user_info']['occupation'])).'" />',
		'interests'        => $lang['Interests'],
		'interests_input'  => '<input type="text" size="50" maxlength="255" name="interests" value="'.htmlentities(stripslashes($session->sess_info['user_info']['interests'])).'" />',
		'signature'        => $lang['Signature'],
		'signature_input'  => '<textarea rows="'.$template->get_config('textarea_rows').'" cols="'.$template->get_config('textarea_cols').'" name="signature">'.htmlentities(stripslashes($session->sess_info['user_info']['signature'])).'</textarea>',
		'msnm'             => $lang['MSNM'],
		'msnm_input'       => '<input type="text" size="50" maxlength="255" name="msnm" value="'.htmlentities(stripslashes($session->sess_info['user_info']['msnm'])).'" />',
		'yahoom'           => $lang['YahooM'],
		'yahoom_input'     => '<input type="text" size="50" maxlength="255" name="yahoom" value="'.htmlentities(stripslashes($session->sess_info['user_info']['yahoom'])).'" />',
		'aim'              => $lang['AIM'],
		'aim_input'        => '<input type="text" size="50" maxlength="255" name="aim" value="'.htmlentities(stripslashes($session->sess_info['user_info']['aim'])).'" />',
		'icq'              => $lang['ICQ'],
		'icq_input'        => '<input type="text" size="50" maxlength="255" name="icq" value="'.htmlentities(stripslashes($session->sess_info['user_info']['icq'])).'" />',
		'jabber'           => $lang['Jabber'],
